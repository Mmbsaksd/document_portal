...
class SingleDocIngestor:
    ...
    def ingest_file(self, uploaded_files):
        try:
            ...
            self.log.error("Failed to initialize SingleDocIngesor", error = str(e))
            raise DocumentPortalExeption("Initialization error in SingleDocIngesion. ", sys)    
    
    def _create_retriever(self, documents):
        try:
            splitter = RecursiveCharacterTextSplitter(chunk_size = 1000, chunk_overlap = 300)
            chunks = splitter.split_documents(documents)
            self.log.info("Document splitted into chunks", count = len(chunks))
